...
List mcmc_Open(NumericVector lam0in, NumericVector sigmain, NumericVector gammain,NumericVector gammaprime, NumericVector phiin,
      ...
      if(primary(l)){
        ...
        for(int i=0; i<M; i++) {
          for(int j=0; j<Xidx(l); j++){
            ...
            if(z(i,l)==1){
              ll_y_curr(i,j,l)=R::dbinom(y(i,j,l),tf(j,l),pd(i,j,l),TRUE);
    ...
    for(int l=0; l<t; l++){
      if(primary(l)){
        ...
        for(int i=0; i<M; i++) {
          for(int j=0; j<Xidx(l); j++){
            if(z(i,l)==1){
              ll_y_curr(i,j,l)=R::dpois(y(i,j,l),tf(j,l)*lamd(i,j,l),TRUE);
              likcurr2D(l)+=ll_y_curr(i,j,l); //year-specific components
            }else{
              ll_y_curr(i,j,l)=0;
            }
  ...
  for(int i=0; i<M; i++){//z1
    ll_z(i,0)=R::dbinom(z(i,0),1,psi,TRUE);
  }
  for(int l=1; l<t; l++){//z2+
    for(int i=0; i<M; i++){
      ll_z(i,l)=R::dbinom(z(i,l),1,Ez(i,l-1),TRUE);
    }
  }
  ...
  for(int iter=0; iter<niter; iter++){
            ...
            for(int i=0; i<M; i++) {
              for(int j=0; j<Xidx(l); j++){
                ...
                if(obstype==1){
                  ...
                  if(z(i,l)==1){
                    ll_y_cand(i,j,l)=R::dbinom(y(i,j,l),tf(j,l),pdcand(i,j,l),TRUE);
                  ...
                  if(z(i,l)==1){
                    ll_y_cand(i,j,l)=R::dpois(y(i,j,l),tf(j,l)*lamdcand(i,j,l),TRUE);
                    likcand2D(l)+=ll_y_cand(i,j,l);
                  }else{
                    ll_y_cand(i,j,l)=0;
                  }
    ...
    if(jointZ==FALSE){
      ...
      for(int i=0; i<M; i++){
        if(upz(i)){
          ...
          for(int j=0; j<Xidx(0); j++){
            if(obstype==1){
              if(z1cand(i)==1){
                ll_y_cand(i,j,0)=R::dbinom(y(i,j,0),tf(j,0),pd(i,j,0),TRUE);
              }else{
                ll_y_cand(i,j,0)=0;
              }
            }else{
              if(z1cand(i)==1){
                ll_y_cand(i,j,0)=R::dpois(y(i,j,0),tf(j,0)*lamd(i,j,0),TRUE);
              }else{
                ll_y_cand(i,j,0)=0;
              }
            }
          ...
          if((((z1cand(i)==1)&(sumz==0))|((z1cand(i)==0)&(z(i,0)==1)&(sumz==1)))&(t>2)){//Are we turning on a guy that was never on before? or turning off a guy that was only on on z1?
            ...
            if(warn(i)==FALSE){
              ll_z_cand(i,0)=R::dbinom(zcand(i,0),1,psi,TRUE);
              llzcandsum=0;
              llzsum=0;
              ...
              for(int l=1; l<t; l++){
                for(int i2=0; i2<M; i2++){
                  ...
                  ll_z_cand(i2,l)=R::dbinom(zcand(i2,l),1,Ezcand(i2,l-1),TRUE);
            ...
            if(warn(i)==FALSE){
              ll_z_cand(i,0) = R::dbinom(z1cand(i),1,psi,TRUE);
              //sum z ll
              llzcandsum=0;
              llzsum=0;
              llzcandsum+=ll_z_cand(i,0);//add ll.z[i,1] for focal guy
              ...
              for(int i2=0; i2<M; i2++){
                ...
                ll_z_cand(i2,1) = R::dbinom(z(i2,1),1,Ezcand(i2,0),TRUE);
          ...
          for(int i=0; i<propzuse; i++){
            ...
            if(primary(l)){
              for(int j=0; j<Xidx(l); j++){
                if(obstype==1){
                  if(zt_cand(swapz(i))==1){
                    ll_y_cand(swapz(i),j,l)=R::dbinom(y(swapz(i),j,l),tf(j,l),pd(swapz(i),j,l),TRUE);
                  }else{
                    ll_y_cand(swapz(i),j,l)=0;
                  }
                }else{
                  if(zt_cand(swapz(i))==1){
                    ll_y_cand(swapz(i),j,l)=R::dpois(y(swapz(i),j,l),tf(j,l)*lamd(swapz(i),j,l),TRUE);
                  }else{
                    ll_y_cand(swapz(i),j,l)=0;
                  }
                }
            ...
            ll_z_cand(swapz(i),l) = R::dbinom(zt_cand(swapz(i)),1,Ez(swapz(i),l-1),TRUE);
              ...
              for(int l2=l; l2<(t-1); l2++){
                ...
                if(gammaprimecand(l2) < 1){ //Is this a valid probability?
                  for(int i2=0; i2<M; i2++){
                    ...
                    ll_z_cand(i2,l2+1) = R::dbinom(z(i2,l2+1),1,Ezcand(i2,l2),TRUE);
      ...
      for(int i=0; i<nzpossible; i++){
        llzpossible(i,0) = R::dbinom(zpossible(i,0),1,psi,TRUE);
      }
      ...
      for(int l=1; l<t; l++){
        for(int i2=0; i2<nzpossible; i2++){
          ...
          llzpossible(i2,l) = R::dbinom(zpossible(i2,l),1,Ezpossible(i2,l-1),TRUE);
        }
      }
      for(int i=0; i<M; i++){
        if(upz3){
            ...
            for(int l=0; l<t; l++){
              if(primary(l)){
                for(int j=0; j<Xidx(l); j++){
                  if(obstype==1){
                    if(zcand(i,l)==1){
                      ll_y_cand(i,j,l)=R::dbinom(y(i,j,l),tf(j,l),pd(i,j,l),TRUE);
                    }else{
                      ll_y_cand(i,j,l)=0;
                    }
                  }else{
                    if(zcand(i,l)==1){
                      ll_y_cand(i,j,l)=R::dpois(y(i,j,l),tf(j,l)*lamd(i,j,l),TRUE);
                    }else{
                      ll_y_cand(i,j,l)=0;
                    }
                  }
      ...
      if((gamma_cand>0)&(gamma_cand_ok)){
        ...
        for(int l=1; l<t; l++){
          for(int i=0; i<M; i++){
            ...
            ll_z_cand(i,l)= R::dbinom(z(i,l),1,Ezcand(i,l-1),TRUE);
...
List mcmc_Open_sex(NumericVector lam0in, NumericVector sigmain, NumericVector gammain,NumericVector gammaprimeM,
  ...
  for(int iter=0; iter<niter; iter++){
    ...
    if(jointZ==FALSE){
      ...
      //ll.z[,1] won't change across i
      for(int i=0; i<nzpossible; i++){
        llzpossibleM(i,0) = R::dbinom(zpossible(i,0),1,psi,TRUE);
        llzpossibleF(i,0) = R::dbinom(zpossible(i,0),1,psi,TRUE);
      }
      //Get likelihood for all possible z histories. must update if accepted
      for(int l=1; l<t; l++){
        for(int i2=0; i2<nzpossible; i2++){
          EzpossibleM(i2,l-1)=zpossible(i2,l-1)*phi(0) + apossible(i2,l-1)*gammaprimeM(l-1);
          llzpossibleM(i2,l)=R::dbinom(zpossible(i2,l),1,EzpossibleM(i2,l-1),TRUE);
          EzpossibleF(i2,l-1)=zpossible(i2,l-1)*phi(1) + apossible(i2,l-1)*gammaprimeF(l-1);
          llzpossibleF(i2,l)=R::dbinom(zpossible(i2,l),1,EzpossibleF(i2,l-1),TRUE);
        }
      }
      for(int i=0; i<M; i++){
        if(upz3){
    ...
    for(int i=0; i<propsex; i++){
        ...
        if(primary(l)){
          for(int j=0; j<Xidx(l); j++){
            ...
            if(obstype==1){
              ...
              if(z(swapsex(i),l)==1){
                ll_y_cand(swapsex(i),j,l)=R::dbinom(y(swapsex(i),j,l),tf(j,l),pdcand(swapsex(i),j,l),TRUE);
              }else{
                ll_y_cand(swapsex(i),j,l)=0;
              }
            }else{
              if(z(swapsex(i),l)==1){
                ll_y_cand(swapsex(i),j,l)=R::dpois(y(swapsex(i),j,l),tf(j,l)*lamdcand(swapsex(i),j,l),TRUE);
              }else{
                ll_y_cand(swapsex(i),j,l)=0;
              }
            }
      ...
      if(gamma_cand_ok&gamma_cand_ok){
        //sex likelihood
        ll_sex_cand(swapsex(i))=R::dbinom(sex_cand(swapsex(i))-1,1,psex,TRUE);
